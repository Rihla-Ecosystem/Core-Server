...
test('7. values above Number.MAX_SAFE_INTEGER stay exact through direct bigint conversion', () => {
  const big = '9000000000000000000';
  const { rows } = convertAdminEntriesForImport([entry(big)], META);
  assert.equal(rows[0].inputMicrosPerMillion, 9_000_000_000_000_000_000n);
  assert.equal(rows[0].inputMicrosPerMillion.toString(), big, 'no precision loss through Number');
});
