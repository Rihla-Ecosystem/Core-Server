...
function relationUserWhere(where: Prisma.UserWhereInput): Prisma.UserRelationFilter {
  return { is: where };
}
...
function getCountByAction(items: Array<{ action: string }>, action: string): number {
...
export async function listUsers(filters: DashboardListFilters): Promise<DashboardUsersListResult> {
  ...
  const [total, rows, totalUsers, activeUsers, verifiedUsers, bannedUsers, deletedUsers, walletUsers, avgUser, walletAggregate, paymentAggregate, tripCount, badgeCount, conversationCount] = await Promise.all([
    ...
    prisma.tokenWallet.count({ where: relationUserWhere(where) }),
    prisma.user.aggregate({
      where,
      _avg: { xp: true, level: true },
    }),
    prisma.tokenWallet.aggregate({
      where: relationUserWhere(where),
  ...
  const averageWalletBalance = decimalToNumber(walletAggregate._avg.tokenBalance);
...
export async function getUserProfile(userId: string): Promise<DashboardUserProfileResult> {
  ...
  return {
    ...
    paymentStatistics: {
      ...
      completedPayments: getCountByAction(paymentStatuses, 'COMPLETED'),
      pendingPayments: getCountByAction(paymentStatuses, 'PENDING'),
      failedPayments: getCountByAction(paymentStatuses, 'FAILED'),
      cancelledPayments: getCountByAction(paymentStatuses, 'CANCELLED'),
      refundedPayments: getCountByAction(paymentStatuses, 'REFUNDED'),
    },
    tokenTransactions,
    journeyProgress: {
      totalJourneys,
...
export async function getDashboardStatistics(filters: Omit<DashboardListFilters, 'page' | 'limit' | 'sort' | 'order'> = {}): Promise<DashboardStatisticsResult> {
  ...
  const [totalUsers, activeUsers, verifiedUsers, bannedUsers, deletedUsers, maleUsers, femaleUsers, walletUsers, averageWallet, totalWallet, paymentStatusCounts, revenueTodayRows, revenueWeekRows, revenueMonthRows, revenueAllRows, topCountriesRows, roleRows, topXpUsers, topTokenWallets, topPayments, topTravelersRows, topConversationRows, totalTrips, totalBadges, totalConversations, userTotals] = await Promise.all([
    ...
    prisma.tokenWallet.count({ where: relationUserWhere(where) }),
    prisma.tokenWallet.aggregate({ where: relationUserWhere(where), _avg: { tokenBalance: true }, _sum: { tokenBalance: true } }),
    prisma.tokenWallet.aggregate({ where: relationUserWhere(where), _sum: { tokenBalance: true } }),
  ...
  const topPayingUsers = topPayments.map((row) => ({
    id: row.userId,
    displayName: topXpUsers.find((user) => user.id === row.userId)?.displayName ?? '',
    email: topXpUsers.find((user) => user.id === row.userId)?.email ?? '',
    avatar: topXpUsers.find((user) => user.id === row.userId)?.avatarUrl ?? null,
  ...
  return {
    ...
    averageWalletBalance: decimalToNumber(averageWallet._avg.tokenBalance),
